...
=item C<json_parser *json_parser_new (fz_context *ctx, fz_stream *stm);>

Return a new parser, which will read from C<stm>. Increase the refcount of
C<stm>. Throw on error.

=item C<void json_parser_drop (fz_context *ctx, json_parser *parser);>

Deallocate C<parser> and decrease the refcount of the underlying stream.

=item C<json_token json_parser_parse (fz_context *ctx, json_parser *parser,
